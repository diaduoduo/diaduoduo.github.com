...
function loadImg(name,n) {
    let arr=[];
    for(let k =0;k<n;k++){
        arr.push("https://apptopgalleria.oss-cn-shanghai.aliyuncs.com/img/"+name.replace("{{}}",addZero(k+1)));
    }
    return arr;
}
function loadImg2(name) {
    return "https://apptopgalleria.oss-cn-shanghai.aliyuncs.com/img/"+name;
}
